...
#Se inicia el clasificador de sentimientos
			...
			for t in tweets:
				#texto = re.sub(r'(https|http)\S+', '', t.text.replace(';',','))
				#texto = re.sub(r'(pic.twitter.com)\S+', '', t.text.replace(';',','))
				#texto = ' '.join(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)"," ",unidecode.unidecode(t.text)).split())
